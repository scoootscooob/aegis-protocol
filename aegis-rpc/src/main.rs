#![allow(dead_code)]
//! Aegis RPC Proxy — The Execution Highway for AI Agent Transactions.
//!
//! Instead of agents broadcasting directly to the public mempool,
//! they point their RPC URL to `https://rpc.aegis.network`.
//!
//! The proxy:
    ...
    fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("aegis_rpc=info,tower_http=debug")),
        )
        .init();

    let cfg = config::Config::from_env()?;
    tracing::info!(
        "Aegis RPC Proxy v{} starting on {}:{}",
