"""
aegis.proxy.http_proxy — Web2 Egress Gateway (Python-side).

A Starlette-based HTTP forward proxy that intercepts outgoing API calls
from AI agents and applies Aegis CapitalVelocity PID enforcement.

Usage (Python-only deployments)::

    HTTP_PROXY=http://127.0.0.1:8080
    uvicorn aegis.proxy.http_proxy:create_http_proxy_app --port 8080

For the full Rust-based TLS proxy, see ``aegis-rpc/src/http_proxy.rs``.
"""
...
from aegis.intent import IntentAction, IntentProtocol, NormalizedIntent

logger = logging.getLogger("aegis.proxy.http")
...
def evaluate_http_request(
    ...
    """Evaluate an HTTP API request through the Aegis firewall.
