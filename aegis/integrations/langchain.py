"""
aegis.integrations.langchain — ``@aegis_tool`` decorator for LangChain.

Slap this decorator on top of any existing LangChain ``@tool`` to
automatically pipe inputs through the 7 Aegis math engines.  If Aegis
...
Usage::
    ...
    @tool
    @aegis_tool(firewall, spend_key="amount")
...
logger = logging.getLogger("aegis.integrations.langchain")
...
def aegis_tool(
    ...
    *,
    ...
    """Decorator that wraps a function with Aegis enforcement.
    ...
    def decorator(fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if verdict.blocked:
                logger.warning(
                    "AEGIS BLOCK in LangChain tool '%s': %s",
                    fn.__name__,
                    verdict.reason,
                )
        ...
        wrapper.aegis_firewall = firewall  # type: ignore[attr-defined]
        return wrapper
