"""
aegis.integrations.automaton — Drop-in Aegis wallet for the Automaton (Conway)
agent framework.
...
Every outgoing transaction is piped through the 7-engine Aegis firewall
*before* execution.  If the firewall blocks, the wallet returns a dict
with ``aegis_blocked: True`` and a cognitive feedback prompt so the LLM
...
logger = logging.getLogger("aegis.integrations.automaton")


@dataclass
class AegisAutomatonWallet:
    """Composition-based Aegis wrapper around an Automaton ``Wallet``.
    ...
    *Why composition, not inheritance?*  Automaton's ``Wallet`` class
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "aegis[automaton] requires the 'automaton' package.  "
                "Install with: pip install aegis-protocol[automaton]"
            )

    # ── Public API ────────────────────────────────────────────────

    def execute(self, payload: dict[str, Any], **kwargs: Any) -> Any:
        """Execute a transaction through Aegis before forwarding to Automaton."""
        spend = float(payload.get("value", payload.get("amount", 0)))
        verdict = self._firewall.evaluate(payload, spend_amount=spend)

        if verdict.blocked:
            logger.warning("AEGIS BLOCK in AutomatonWallet: %s", verdict.reason)
            return {
                "aegis_blocked": True,
    ...
    @property
    def firewall(self) -> AegisFirewall:
