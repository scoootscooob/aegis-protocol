"""
``aegis up``  — Start the Dockerized Aegis RPC Proxy.
``aegis status`` — Ping the proxy health endpoint.

Locates ``docker-compose.yml`` automatically (project root → package
directory fallback), optionally loads ``aegis.toml`` into the
...
_COMPOSE_CANDIDATES = [
    ...
    os.path.join("aegis-rpc", "docker-compose.yml"),
    "docker-compose.yml",
    # Relative to this Python file (package install)
    os.path.join(os.path.dirname(__file__), "..", "..", "aegis-rpc", "docker-compose.yml"),
]
...
def _load_toml_env(path: str) -> None:
    ...
    ``[velocity]`` + ``v_max = 0.01`` → ``AEGIS_VELOCITY_V_MAX=0.01``.
    """
    section = "aegis"
    with open(path) as fh:
        for line in fh:
            ...
            env_key = f"AEGIS_{section.upper()}_{key.upper()}" if section != "aegis" else f"AEGIS_{key.upper()}"
...
def run_up(
    ...
    """Start the Aegis RPC Proxy via ``docker compose``."""
    cfile = compose_file or _find_compose_file()
    if cfile is None:
        print(
            "Error: docker-compose.yml not found.\n"
            "Run `aegis up` from the project root, or pass --compose-file."
        )
        sys.exit(1)

    # Optionally load aegis.toml
    toml_path = os.path.join(os.getcwd(), "aegis.toml")
    ...
    print(f"  Starting Aegis RPC Proxy ({cfile}) ...")
    try:
...
def run_status(host: str = "localhost", port: int = 8545) -> None:
    ...
    try:
        ...
        print(f"  \u274c  Aegis RPC Proxy not reachable at {url}: {exc}")
        sys.exit(1)
