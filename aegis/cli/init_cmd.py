"""
``aegis init`` — Interactive 3-question wizard that generates ``aegis.toml``.

The generated file is human-readable TOML consumed by both the Python SDK
(``AegisConfig``) and the Rust RPC Proxy (via env-var expansion in
``docker-compose.yml``).
"""
...
# Docs: https://docs.aegis-protocol.dev
...
def run_init(output_dir: str | None = None) -> str:
    """Interactive wizard that writes ``aegis.toml``.

    Returns the absolute path of the generated file (useful for testing).
    """
    print("\n  \033[1mAegis Protocol \u2014 Configuration Wizard\033[0m\n")
    ...
    output_path = os.path.join(base, "aegis.toml")
    with open(output_path, "w") as fh:
        fh.write(content)

    print(f"\n  \u2705  Written to {output_path}")
    print("  Next: run \033[1maegis up\033[0m to start the RPC proxy\n")
    return output_path
